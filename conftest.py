...
# (autouse=True) for yandex driver (если используется другой браузер нужно выбрать соответствующую фикстуру ниже)
def driver():  # при использовании закомментить строку browser = webdriver.Chrome(executable_path= в def web_browser
    s = Service(r"C:\Users\User\Desktop\Lern\yandexdriver.exe")  # локальный путь к драйверу
    driver = webdriver.Chrome(service=s)  # Инициализируем драйвер
    chromeOptions = webdriver.ChromeOptions()  # Создаем объект options
    chromeOptions.binary_location = r"C:\Users\User\Desktop\Lern\browser.exe"
    # driver.get('https://b2c.passport.rt.ru/')  # Переходим на страницу авторизации (pytest.driver)
...
# def web_browser(request):
    # browser = webdriver.Firefox(executable_path=r"C:\Users\User\Desktop\Lern\geckodriver.exe")
    # for firefox driver
    # browser = webdriver.Chrome(executable_path=r"C:\Users\User\Desktop\Lern\chromedriver.exe")
    # for chrome driver
